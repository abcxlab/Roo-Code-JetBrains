...
import com.roocode.jetbrains.actors.MainThreadClipboard
...
class TerminalAPICommands(val project: Project) {
    ...
    suspend fun workbench_action_terminal_copySelection(): Any? {
        logger.debug("Copying terminal output to clipboard")
        ...
        if (textToCopy.isNotEmpty()) {
            logger.debug("Successfully copied terminal output to clipboard")
        } else {
            logger.debug("Copied empty terminal output to clipboard")
        }
