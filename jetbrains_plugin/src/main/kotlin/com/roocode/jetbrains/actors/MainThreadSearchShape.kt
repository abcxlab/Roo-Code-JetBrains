...
class MainThreadSearch : MainThreadSearchShape {
    ...
    override fun registerFileSearchProvider(handle: Int, scheme: String) {
        try {
            logger.debug("Registering file search provider: handle=$handle, scheme=$scheme")
    ...
    override fun registerAITextSearchProvider(handle: Int, scheme: String) {
        try {
            logger.debug("Registering AI text search provider: handle=$handle, scheme=$scheme")
    ...
    override fun registerTextSearchProvider(handle: Int, scheme: String) {
        try {
            logger.debug("Registering text search provider: handle=$handle, scheme=$scheme")
    ...
    override fun unregisterProvider(handle: Int) {
        try {
            logger.debug("Unregistering provider: handle=$handle")
    ...
    override fun handleFileMatch(handle: Int, session: Int, data: List<Map<String, Any?>>) {
        try {
            logger.debug("Handling file match: handle=$handle, session=$session, matches=${data.size}")
    ...
    override fun handleTextMatch(handle: Int, session: Int, data: List<Map<String, Any?>>) {
        try {
            logger.debug("Handling text match: handle=$handle, session=$session, matches=${data.size}")
    ...
    override fun handleTelemetry(eventName: String, data: Any?) {
        try {
            logger.debug("Handling telemetry: event=$eventName, data=$data")
        } catch (e: Exception) {
    ...
    override fun dispose() {
        logger.debug("Disposing MainThreadSearch")
