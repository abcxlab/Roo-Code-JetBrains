...
class TerminalShellIntegration(
    ...
    fun setupShellIntegration() {
        runCatching {
            logger.debug("$LOG_PREFIX_SETUP Setting up shell integration (terminal: $extHostTerminalId)...")
            
            initializeShellEventListener()
            initializeShellIntegrationState()
            
            logger.debug("$LOG_PREFIX_SUCCESS Shell integration setup complete (terminal: $extHostTerminalId)")
        }.onFailure { exception ->
     ...
     */
    fun dispose() {
        logger.debug("$LOG_PREFIX_DISPOSE Disposing shell integration: $extHostTerminalId")
        ...
        runCatching {
            ...
            logger.debug("$LOG_PREFIX_SUCCESS Shell integration disposed: $extHostTerminalId")
        }.onFailure { exception ->
    ...
    private inner class TerminalShellEventListener : ShellEventListener {
        ...
        override fun onShellExecutionStart(commandLine: String, cwd: String) {
            logger.debug("$LOG_PREFIX_START Command execution started: '$commandLine' in directory '$cwd' (terminal: $extHostTerminalId)")
        ...
        override fun onShellExecutionEnd(commandLine: String, exitCode: Int?) {
            ...
            logger.debug("$LOG_PREFIX_END Command execution finished: '$commandLine' (exit code: $actualExitCode) (terminal: $extHostTerminalId)")
        ...
        override fun onCwdChange(cwd: String) {
            logger.debug("$LOG_PREFIX_CWD Working directory changed to: '$cwd' (terminal: $extHostTerminalId)")
