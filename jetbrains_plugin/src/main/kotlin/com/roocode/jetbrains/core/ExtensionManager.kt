...
class ExtensionManager : Disposable {
    private val logger = Logger.getInstance(ExtensionManager::class.java)
    ...
    fun activateExtension(extensionId: String, rpcProtocol: IRPCProtocol): CompletableFuture<Boolean> {
        ...
        try {
            ...
            try {
                ...
                logger.error("Failed to call activate method: $extensionId", e)
                val future = CompletableFuture<Boolean>()
            ...
            logger.error("Failed to activate extension: $extensionId", e)
            val future = CompletableFuture<Boolean>()
            future.completeExceptionally(e)
            return future
        }
    ...
    override fun dispose() {
        logger.info("Releasing ExtensionManager resources")
        extensions.clear()
    }
}
