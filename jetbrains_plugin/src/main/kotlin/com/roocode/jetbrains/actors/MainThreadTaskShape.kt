...
class MainThreadTask : MainThreadTaskShape {
    ...
    override fun createTaskId(task: Map<String, Any?>):String {
        try {
            logger.debug("Creating task ID for task: $task")
    ...
    override fun registerTaskProvider(handle: Int, type: String) {
        try {
            logger.debug("Registering task provider: handle=$handle, type=$type")
    ...
    override fun unregisterTaskProvider(handle: Int) {
        try {
            logger.debug("Unregistering task provider: handle=$handle")
    ...
    override fun fetchTasks(filter: Map<String, Any?>?): List<Map<String, Any?>> {
        try {
            logger.debug("Fetching tasks with filter: $filter")
    ...
    override fun getTaskExecution(value: Map<String, Any?>): Map<String, Any?> {
        try {
            ...
            logger.debug("Getting task execution for task: $taskId")
    ...
    override fun terminateTask(id: String) {
        try {
            logger.debug("Terminating task: $id")
    ...
    override fun registerTaskSystem(scheme: String, info: Map<String, Any?>) {
        try {
            logger.debug("Registering task system: scheme=$scheme, info=$info")
    ...
    override fun customExecutionComplete(id: String, result: Int?) {
        try {
            logger.debug("Custom execution complete for task: $id with result: $result")
    ...
    override fun registerSupportedExecutions(custom: Boolean?, shell: Boolean?, process: Boolean?) {
        try {
            logger.debug("Registering supported executions: custom=$custom, shell=$shell, process=$process")
        } catch (e: Exception) {
    ...
    override fun dispose() {
        logger.debug("Disposing MainThreadTask")
        taskProviders.clear()
