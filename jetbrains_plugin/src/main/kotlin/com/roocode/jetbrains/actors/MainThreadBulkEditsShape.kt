...
import com.roocode.jetbrains.editor.EditorHolder
import com.roocode.jetbrains.editor.WorkspaceEdit
...
class MainThreadBulkEdits(val project: Project) : MainThreadBulkEditsShape {
    ...
    override suspend fun tryApplyWorkspaceEdit(workspaceEditDto: SerializableObjectWithBuffers<Any>, undoRedoGroupId: Int?, respectAutoSaveConfig: Boolean?): Boolean {
        ...
        logger.debug("[Bulk Edit] Starting process: $json")
        ...
        cto.files.forEach { fileEdit ->
            if (fileEdit.oldResource != null && fileEdit.newResource != null) {
                ...
                try {
                    ...
                    logger.debug("[Bulk Edit] Renamed file: ${oldResource.path} -> ${newResource.path}")
                } catch (e: Exception) {
                ...
                try {
                    ...
                    logger.debug("[Bulk Edit] Deleted file: ${oldResource.path}")
                } catch (e: Exception) {
                ...
                try {
                    ...
                    logger.debug("[Bulk Edit] Created file: ${newResource.path}")
                } catch (e: Exception) {
        ...
        cto.texts.forEach { textEdit ->
            logger.debug("[Bulk Edit] Processing text edit: ${textEdit.resource.path}")
            ...
            try {
                ...
                logger.debug("[Bulk Edit] Failed to get editor handle: ${textEdit.resource.path}", e)
            }

            if (handle == null) {
                logger.debug("[Bulk Edit] Editor handle not found: ${textEdit.resource.path}")
                allSuccess = false
                return@forEach
            }
            ...
            try {
                ...
                if (!result) {
                    logger.debug("[Bulk Edit] Failed to apply edit: ${textEdit.resource.path}")
                    allSuccess = false
                } else {
                    logger.debug("[Bulk Edit] Successfully updated file: ${textEdit.resource.path}")
                }
            } catch (e: Exception) {
