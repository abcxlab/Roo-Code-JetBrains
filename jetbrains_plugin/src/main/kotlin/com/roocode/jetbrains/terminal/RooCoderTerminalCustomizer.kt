...
class RooCoderTerminalCustomizer : LocalTerminalCustomizer() {
  ...
  private fun copyShellIntegrationFiles() {
    ...
    ApplicationManager.getApplication().executeOnPooledThread {
      ...
      try {
        logger.debug("🚀 Start async copy of shell integration files to user home...")
        ...
        shellConfigs.forEach { (shellType, files) ->
          ...
          if (!targetDirFile.exists()) {
            ...
            logger.debug("📁 Created $shellType target directory: $targetDir")
          }
          ...
          files.forEach { fileName ->
            ...
            if (inputStream != null) {
              ...
              logger.debug("✅ Successfully copied $shellType file: $fileName")
            } else {
        ...
        logger.debug("✅ Shell integration files async copy complete")
  ...
  private fun injectVSCodeScript(command: Array<String>, envs: MutableMap<String, String>): Array<String> {
    ...
    logger.debug("🔧 Injecting Shell Integration script: $scriptPath")
    logger.debug("🐚 Shell type: $shellName")
  ...
  private fun injectBashScript(command: Array<String>, envs: MutableMap<String, String>, scriptPath: String): Array<String> {
    ...
    return if (rcfileIndex != -1 && rcfileIndex + 1 < command.size) {
      ...
      logger.debug("🔧 Detected existing --rcfile parameter: $originalRcfile")
      ...
      logger.debug("🔧 Replaced --rcfile parameter with: $scriptPath")
      newCommand
    } else {
      // If --rcfile parameter does not exist, add new parameter
      logger.debug("🔧 Added new --rcfile parameter: $scriptPath")
  ...
  private fun injectZshScript(command: Array<String>, envs: MutableMap<String, String>, scriptPath: String): Array<String> {
    ...
    if (jetbrainsZshDir != null) {
      ...
      logger.debug("🔧 Preserved JETBRAINS_INTELLIJ_ZSH_DIR: $jetbrainsZshDir")
    }

    envs["USER_ZDOTDIR"] = userZdotdir
    envs["ZDOTDIR"] = scriptPath
    
    logger.debug("🔧 Saved original ZDOTDIR: $userZdotdir, set new ZDOTDIR: $scriptPath")
    return command
  }
  ...
  private fun injectPowerShellScript(command: Array<String>, envs: MutableMap<String, String>, scriptPath: String): Array<String> {
    ...
    logger.debug("🔧 Set PowerShell environment variables: VSCODE_NONCE=${envs["VSCODE_NONCE"]}")
    ...
    return if (fileIndex != -1 && fileIndex + 1 < command.size) {
      ...
      logger.debug("🔧 Detected existing -File parameter: $originalScript")
      ...
      logger.debug("🔧 Replace -File parameter with: $scriptPath")
      newCommand
    } else {
      ...
      logger.debug("🔧 Add new -File parameter: $scriptPath")
