...
package com.roocode.jetbrains.ipc.proxy
...
import com.roocode.jetbrains.ipc.proxy.uri.UriReplacer
...
class RPCProtocol(
    ...
    private fun onDidReceiveAcknowledge(req: Int) {
       ...
       if (unacknowledgedCount > 0) {
           ...
           LOG.warn("Received unexpected acknowledge when unacknowledged count is already 0 or negative, request ID: $req, current unacknowledged count: $unacknowledgedCount")
        }
//        LOG.debug("Received acknowledge, request ID: $req, unacknowledged count decreased: $unacknowledgedCount, updated unresponsive time: ${unresponsiveTime}ms")
    ...
    private fun setResponsiveState(newResponsiveState: ResponsiveState) {
        ...
        LOG.debug("Responsive state changed from $responsiveState to $newResponsiveState")
...
//            throw err
    ...
    private suspend fun doInvokeHandler(rpcId: Int, methodName: String, args: List<Any?>): Any? {
        ...
        if (actor == null) {
            ...
            LOG.warn("Unknown actor $actorName called with method $methodName, returning null for compatibility")
            return null
        }
       ...
       val method = try {
           ...
           LOG.warn("Unknown method $methodName on actor ${getStringIdentifierForProxy(rpcId)}, returning null for compatibility", e)
           return null
       }
