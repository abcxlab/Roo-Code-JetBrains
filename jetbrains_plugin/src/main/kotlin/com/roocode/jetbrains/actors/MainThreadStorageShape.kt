...
import com.roocode.jetbrains.service.ExtensionStorageService
...
class MainThreadStorage : MainThreadStorageShape {
    ...
    override fun initializeExtensionStorage(shared: Boolean, extensionId: String): Any? {
        logger.debug("Initializing extension storage: shared=$shared, extensionId=$extensionId")
...
//        logger.debug("Setting value: shared=$shared, extensionId=$extensionId, value=$value")
        val storage = service<ExtensionStorageService>()
    ...
    override fun registerExtensionStorageKeysToSync(extension: Any, keys: List<String>) {
        ...
        logger.debug("Registering extension storage keys for sync: extension=$extensionId, keys=$keys")
    }

    override fun dispose() {
        logger.debug("Dispose MainThreadStorage")
    }
} 
