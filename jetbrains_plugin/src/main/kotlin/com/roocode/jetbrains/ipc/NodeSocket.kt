...
class NodeSocket : ISocket {
                ...
                while (!isDisposed.get() && !Thread.currentThread().isInterrupted) {
                    try {
                        ...
                        if (!isDisposed.get()) {
                            ...
                            if (e is SocketException && e.message?.contains("Connection reset") == true) {
                                logger.info("Socket[$debugLabel] Connection reset by peer (normal shutdown)")
                                handleSocketError(e)
                            } else {
    ...
    override fun dispose() {
        ...
        logger.debug("Socket[$debugLabel] Releasing resources")
        ...
        logger.debug("Socket[$debugLabel] Resource release completed")
    }
