...
class MainThreadFileSystemEventService : MainThreadFileSystemEventServiceShape {
    ...
    override fun watch(
        ...
        logger.debug("Starting to watch file system changes: extensionId=$extensionId, session=$session, resource=$resource, opts=$opts, correlate=$correlate")
    }
    ...
    override fun unwatch(session: Int) {
        logger.debug("Stopping file system watch: session=$session")
    }
    ...
    override fun dispose() {
        logger.debug("Releasing MainThreadFileSystemEventService resources")
    }
} 
