...
package com.roocode.jetbrains.workspace
...
import com.roocode.jetbrains.core.WorkspaceManager
import com.roocode.jetbrains.events.*
import com.roocode.jetbrains.ipc.proxy.interfaces.FileSystemEvents
...
class WorkspaceFileChangeManager(val project: Project) : Disposable {
    ...
    private fun triggerWorkspaceRootChangeEvent(project: Project, oldPath: String?, newPath: String) {
        ...
        val extHostWorkspace = project.getService(PluginContext::class.java).getRPCProtocol()?.getProxy(ServiceProxyRegistry.ExtHostContext.ExtHostWorkspace)
    ...
    private fun registerFileListener(project: Project) {
        if (vfsConnections.containsKey(project)) {
            logger.debug("File listener for project ${project.name} already exists, skip registration")
            return
        }

        logger.debug("Register file listener for project ${project.name}")
    ...
    private fun unregisterFileListener(project: Project) {
        ...
        if (connection != null) {
            logger.debug("Unregister file listener for project ${project.name}")
    ...
    private fun triggerBulkFileChangeEvent(fileChanges: List<WorkspaceFileChangeData>, project: Project) {
        ...
        val proxy = project.getService(PluginContext::class.java).getRPCProtocol()?.getProxy(ServiceProxyRegistry.ExtHostContext.ExtHostFileSystemEventService)
        proxy?.let {
    ...
    override fun dispose() {
        logger.debug("Release workspace file change manager resources")
