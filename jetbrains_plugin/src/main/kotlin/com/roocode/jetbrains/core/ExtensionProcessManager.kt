...
class ExtensionProcessManager : Disposable {
    ...
    fun start(portOrPath: Any?): Boolean {
        if (isRunning) {
            logger.info("Extension process is already running")
            return true
        }
        val isUds = portOrPath is String
        if (!ExtensionUtils.isValidPortOrPath(portOrPath)) {
            logger.error("Invalid socket info: $portOrPath")
            return false
        }
        ...
        try {
            ...
            if (nodePath == null) {
                logger.error("Failed to find Node.js executable")
            ...
            if (!NodeVersionUtil.isVersionSupported(nodeVersion, MIN_REQUIRED_NODE_VERSION)) {
                logger.error("Node.js version is not supported: $nodeVersion, required: $MIN_REQUIRED_NODE_VERSION")
            ...
            if (extensionPath == null) {
                logger.error("Failed to find extension entry file")
                return false
            }

            logger.info("Starting extension process with node: $nodePath, entry: $extensionPath")
...
//            }
            ...
            isRunning = true
            logger.info("Extension process started")
            return true
        } catch (e: Exception) {
    ...
    private fun monitorProcess() {
        ...
        try {
            ...
            val logThread = Thread {
                proc.inputStream.bufferedReader().use { reader ->
                    ...
                    while (reader.readLine().also { line = it } != null) {
                        logger.debug("Extension process: $line")
                    }
                }
            }
            ...
            try {
                ...
                logger.debug("Extension process exited with code: $exitCode")
            ...
            logger.error("Error monitoring extension process", e)
        } finally {
            synchronized(this) {
                if (process === proc) {
                    isRunning = false
    ...
    private fun stopInternal() {
        logger.info("Stopping extension process")
        ...
        if (proc != null) {
            try {
                ...
                logger.error("Error stopping extension process", e)
            }
        }
        ...
        logger.info("Extension process stopped")
    }
    ...
    private fun findExecutableInPath(name: String): String? {
        ...
        logger.info("System Node path: $nodePath")
        return nodePath
    }
    ...
    fun findExtensionEntryFile(): String? {
        ...
        if (RooCoderPluginService.getDebugMode() != DEBUG_MODE.NONE) {
            ...
            if (debugEntry.exists() && debugEntry.isFile) {
                ...
                logger.warn("[DebugMode] Debug entry file not found: ${debugEntry.absolutePath}")
            }
        }
        // Normal mode
        val resourcesPath = com.roocode.jetbrains.util.PluginResourceUtil.getResourcePath(PLUGIN_ID, "$RUNTIME_DIR/$EXTENSION_ENTRY_FILE")
        if (resourcesPath != null) {
    ...
    private fun findPluginCodeDir(): String? {
        ...
        logger.warn("Plugin code directory not found")
        return null
    }
    ...
    private fun buildEnhancedPath(envVars: MutableMap<String, String>, nodePath: String): String {
        ...
        commonDevPaths.forEach { path ->
            if (File(path).exists() && !currentPath.contains(path)) {
                ...
                logger.info("Add path to PATH: $path")
            } else if (!File(path).exists()) {
                logger.warn("Path does not exist, skip: $path")
            }
        }
