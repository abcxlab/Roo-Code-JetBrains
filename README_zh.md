...
> **Roo Code for JetBrains (CE) 是一个独立的社区项目，与 JetBrains 或 RooCode, Inc. 无关，也不受其认可或赞助。**
>
...
虽然官方提供了一个品牌化的版本，但它仅仅是 RunVSAgent 的简单换壳，保留了大量未修复的底层 Bug。本社区版 (**Roo Code (CE)**) 进行了深度重构和优化，致力于提供一个真正可用于生产环境的体验：
...
### [从 JetBrains Marketplace 下载](https://plugins.jetbrains.com/plugin/30824-roo-code-ce)（推荐）
...
1. **在线安装**：
   ...
   * 转到 `设置/首选项` → `插件`
   * 在 `Marketplace` 标签页中搜索 "Roo Code (CE)"
   * 点击 `安装` 按钮
   * 在提示时重启您的 IDE

2. **验证安装**：重启后，您应该在 IDE 的插件列表中看到 Roo Code (CE) 插件

### 从 GitHub Releases 下载

您可以从我们的 GitHub releases 页面下载预构建的插件：

1. **下载插件**：访问 [GitHub Releases](https://github.com/abcxlab/Roo-Code-JetBrains-CE/releases) 页面并下载最新的插件文件（`.zip` 格式）
...
3. **验证安装**：重启后，您应该在 IDE 的插件列表中看到 Roo Code (CE) 插件
...
有关已知问题 and 常见问题的列表, 请参见[已知问题](docs/KNOWN_ISSUES.md)。
