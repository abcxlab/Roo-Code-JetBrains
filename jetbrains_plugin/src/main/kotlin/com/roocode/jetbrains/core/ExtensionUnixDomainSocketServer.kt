...
class ExtensionUnixDomainSocketServer : ISocketServer {
    ...
    override fun start(projectPath: String): String? {
        if (isRunning) {
            logger.debug("UDS server is already running")
    ...
    private fun startUds(): String? {
        try {
            ...
            isRunning = true
            logger.debug("[UDS] Listening on: $sockPath")
    ...
    override fun stop() {
        if (!isRunning) return
        isRunning = false
        logger.debug("Stopping UDS socket server")
        ...
        udsSocketPath = null
        logger.debug("UDS socket server stopped")
    }
    ...
    private fun acceptUdsConnections() {
        ...
        logger.debug("[UDS] Waiting for connections..., tid: ${Thread.currentThread().id}")
        while (isRunning && !Thread.currentThread().isInterrupted) {
            try {
                val clientChannel = server.accept() // Block and wait for new connection
                logger.debug("[UDS] New client connected")
                ...
                if (isRunning) {
                    ...
                    logger.debug("[UDS] Accept loop exiting (server stopped)")
                    break
                }
            }
        }
        logger.debug("[UDS] Accept loop terminated.")
    }
        ...
        try {
            ...
            while (clientChannel.isConnected && clientChannel.isOpen && isRunning) {
                try {
                    ...
                    logger.debug("[UDS] Client handler interrupted, exiting loop")
                    break
                }
            }
        } catch (e: Exception) {
            if (e !is InterruptedException) {
                ...
                logger.debug("[UDS] Client handler interrupted during processing")
            }
        } finally {
            ...
            logger.debug("[UDS] Client channel closed and removed.")
        }
    }
