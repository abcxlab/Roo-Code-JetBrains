...
class ExtensionSocketServer() : ISocketServer {
    ...
    override fun start(projectPath: String): Int {
        if (isRunning) {
            logger.debug("Socket server is already running")
    ...
    override fun stop() {
        ...
        logger.debug("Socket server stopped")
    }
    ...
    private fun acceptConnections() {
        ...
        logger.debug("Socket server started, waiting for connections..., tid: ${Thread.currentThread().id}")
        
        while (isRunning && !Thread.currentThread().isInterrupted) {
            try {
                val clientSocket = server.accept()
                logger.debug("New client connected from: ${clientSocket.inetAddress.hostAddress}")
                ...
                logger.debug("Socket server thread interrupted")
                break
            } catch (e: Exception) {
                ...
                if (isRunning) {
                    try {
                        ...
                        logger.debug("Socket server thread interrupted during sleep")
                        break
                    }
                }
            }
        ...
        logger.debug("Socket accept loop terminated")
    }
        ...
        try {
            ...
            while (clientSocket.isConnected && !clientSocket.isClosed && isRunning) {
                try {
                    ...
                    logger.debug("Client handler thread interrupted, exiting loop")
                    break
                }
            }
        } catch (e: Exception) {
            ...
            if (e !is InterruptedException) {
                ...
                logger.debug("Client handler thread interrupted during processing")
            }
        } finally {
            // Clean up resources
            manager.dispose()
            ...
            logger.debug("Client socket closed and removed")
        }
    }
    ...
    fun connectToDebugHost(host: String, port: Int): Boolean {
        if (isRunning) {
            logger.debug("Socket server is already running, stopping first")
            stop()
        }
        
        try {
            logger.debug("Connecting to debug host at $host:$port")
            ...
            logger.debug("Successfully connected to debug host at $host:$port")
            return true
        } catch (e: Exception) {
