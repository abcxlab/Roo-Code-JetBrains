...
class DragDropHandler(
    ...
    fun setupDragAndDrop() {
        logger.debug("Setting up drag and drop support for WebView (VSCode-compatible)")
        ...
        val dropTarget = DropTarget(targetComponent, object : DropTargetAdapter() {
            ...
            override fun dragEnter(dtde: DropTargetDragEvent) {
                ...
                if (isShiftKeyPressed(dtde) && hasFileList(dtde)) {
                    ...
                    logger.debug("Drag accepted - Shift key pressed and files detected")
                } else {
                    dtde.rejectDrag()
                    logger.debug("Drag rejected - ${if (!isShiftKeyPressed(dtde)) "Shift key not pressed" else "no files detected"}")
                }
            }
            ...
            override fun dragExit(dte: DropTargetEvent) {
                logger.debug("Drag exit detected")
        ...
        logger.debug("Drag and drop setup completed")
    }
    ...
    private fun handleFileDrop(dtde: DropTargetDropEvent) {
        try {
            logger.debug("Processing drop event")

            if (!hasFileList(dtde)) {
                logger.debug("Drop rejected: No file list in transferable")
                dtde.rejectDrop()
                notifyDragState(false)
                return
            }
            ...
            logger.debug("Files dropped: ${fileList.map { it.absolutePath }}")
    ...
    private fun insertFilePathsIntoTextarea(files: List<File>) {
        try {
            // Normalize all path separators to forward slashes for cross-platform compatibility
            // in the webview context. This is more robust than just escaping backslashes.
            val filePaths = files.map { file ->
                file.absolutePath.replace("\\", "/")
            }

            logger.debug("Forwarding drag drop event to VSCode native handler: ${filePaths.size} files")
