...
show_help() {
    cat << EOF
$SCRIPT_NAME - Build Roo Code for JetBrains project components

USAGE:
    $SCRIPT_NAME [OPTIONS] [TARGET]

DESCRIPTION:
    This script builds the Roo Code for JetBrains project components:
...
validate_build_environment() {
    ...
    if command_exists "git-lfs"; then
        ...
        log_warn "git-lfs command not found, skipping LFS pull. Large files might be missing."
    fi
...
build_idea_component() {
    ...
    # Clean up previous zip artifacts to ensure a clean state for renaming
    local idea_dist_dir="$PROJECT_ROOT/$IDEA_DIR/build/distributions"
    if [[ -d "$idea_dist_dir" ]]; then
        log_info "Cleaning previous IDEA plugin artifacts from $idea_dist_dir"
        execute_cmd "find '$idea_dist_dir' -name '*.zip' -type f -delete" "Clean IDEA artifacts"
    fi
