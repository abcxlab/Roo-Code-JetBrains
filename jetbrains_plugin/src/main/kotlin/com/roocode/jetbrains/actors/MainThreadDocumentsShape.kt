...
import com.roocode.jetbrains.service.DocumentSyncService
...
class MainThreadDocuments(var project: Project) : MainThreadDocumentsShape {
    ...
    private fun setupDocumentSaveListener() {
        try {
            ...
            logger.debug("Document save listener registered successfully")
        } catch (e: Exception) {
    ...
    private fun handleDocumentSaving(document: Document) {
        ...
        logger.debug("Handle document save event: ${virtualFile?.path}")
    ...
    override suspend fun tryOpenDocument(map: Map<String, Any?>, options: Map<String, Any?>?): Map<String, Any?> {
        ...
        logger.debug("tryOpenDocument : ${uri.path} execution completed")
        return map
    }
    ...
    override suspend fun trySaveDocument(map: Map<String, Any?>): Boolean {
        ...
        logger.debug("trySaveDocument： ${uri.path}")

        project.getService(EditorAndDocManager::class.java).getEditorHandleByUri(uri,true)?.updateDocumentDirty(false) ?: run {
            logger.debug("trySaveDocument： ${uri.path} not found")
            return false
        }
        logger.debug("trySaveDocument： ${uri.path} execution completed")
        return true
    }
    ...
    fun dispose() {
        try {
            ...
            logger.debug("Document save listener disposed")
        } catch (e: Exception) {
