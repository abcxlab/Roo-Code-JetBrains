...
package com.roocode.jetbrains.service
...
import com.roocode.jetbrains.editor.ModelAddedData
...
class DocumentSyncService(private val project: Project) {
    ...
    suspend fun syncDocumentStateOnSave(virtualFile: VirtualFile, document: Document) {
        logger.debug("Starting to sync document save state: ${virtualFile.path}")
        try {
            // Create URI object
            val uriMap = mapOf(
                "scheme" to "file",
            ...
            if (editorHandles.isNotEmpty()) {
                ...
                getExtHostDocumentsProxy()?.let { proxy ->
                    ...
                    logger.debug("Document save event and state synced to extension host: ${virtualFile.path}")
                }
            }
        } catch (e: Exception) {
